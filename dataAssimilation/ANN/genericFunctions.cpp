...
void genericFunctions::perturbKernSmooth(double *vect1, int length, double dirac){
...
void genericFunctions::variogram(double *x, double *y, double *z, double *variance, double *bincent, double *tObs, int length, int bins){
	...
	double binWidth = maxDist/static_cast<double>(bins);

	for (int i = 0; i < bins; i++){
		bincent[i] = (binWidth*i)+(0.5*binWidth);
	}
	...
	for (int i = 0; i < length; i++){
		for (int j = 0; j < length; j++){
			double distance = ((x[i]-x[j])*(x[i]-x[j]))+((y[i]-y[j])*(y[i]-y[j]));
			distance = sqrt(distance);
			int bin = distance/binWidth;
			variance[bin] += (z[i]-z[j])*(z[i]-z[j]);
			tObs[bin] += 1;
		}
	}
	...
	for (int i = 0; i < bins; i++){
		if (tObs[i] > 0){
			variance[i] /= static_cast<double>(tObs[i]);
		}
		else {
			variance[i] = -9999;
		}
